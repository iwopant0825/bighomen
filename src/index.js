...
if (WEBGL.isWebGLAvailable()) {
  ...
  scene.background = new THREE.Color(0x191C00);
  ...
  lodingm.onLoad = () => {
    setTimeout(function() {loding_r.style.display = 'none'; }, 2000);
  };
  ...
  let whaleAnimationMixer = null;
  let isWhaleRotating = false;
  let speed = 0.001;
  ...
  loader.load('./static/models/whale.glb', (gltf) => {
    ...
    whaleModel.scale.set(100, 100, 100);


    // 모델을 오른쪽으로 이동시키기 위한 벡터
    const moveRightVector = new THREE.Vector3(20, 30, 0);
    whaleModel.position.add(moveRightVector);
...
const numInstances = 15; // 생성할 인스턴스 개수
...
function transitionBackground(currentColor, targetColor) {
  var duration = 1000; // 전환 시간(밀리초)
  ...
  function updateColor() {
    ...
    if (elapsedTime >= duration) {
      ...
      var interpolatedColor = new THREE.Color().setHex(currentColor).lerp(new THREE.Color().setHex(targetColor), progress);
...
function fadeOutInstance(instance) {
  ...
  function updateOpacity(timestamp) {
    ...
    if (progress < 1) {
      ...
      // 인스턴스를 씬에서 제거
      instance.position.set(1000, 1000, 1000);
  ...
  con+=1;
  if (con == 2) {
    var currentColor = scene.background.getHex();
    var targetColor = 0x485400;
    transitionBackground(currentColor, targetColor);
  } else if (con == 3) {
    var currentColor = scene.background.getHex();
    var targetColor = 0x006899;
    transitionBackground(currentColor, targetColor);
  } else if (con == 4) {
    var currentColor = scene.background.getHex();
    var targetColor = 0x57DBD9;
    transitionBackground(currentColor, targetColor);
  } else if (con == 5) {
    var currentColor = scene.background.getHex();
    var targetColor = 0xC4ECFC;
    transitionBackground(currentColor, targetColor);
    setTimeout(function() {
